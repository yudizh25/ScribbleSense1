...
class TEXTure:
    ...
    def full_eval(self, output_dir: Path = None):
        ...
        self.evaluate(self.dataloaders['val_large'], output_dir, save_as_video=False)#save_as_video控制是否在result文件夹生成视频
        # except:
    ...
    #     # ================== 修改 1: 强制视角对齐 (Viewpoint Alignment) ==================
    #     # 设置涂鸦对应的 Step 和 验证集索引 (从文件名 step_00002_0002_rgb.jpg 可知)
    #     target_scribble_step = 2        # 对应 step_00002
    #     scribble_val_index = 2          # 对应 _0002 (验证集图片的索引)
    #     scribble_image_path = "/data/tangyangguang/tangyangguang/TEXTurePaper_modified_for_bunny_v2/experiments/2025_12_14_scribble_on_bunny_v3_seed_2_notvideo_havetexture/vis/step_00002_0002_scribble.jpg" # 你的涂鸦文件路径

    #     if self.paint_step == target_scribble_step:
    #         logger.info(f"Detected Target Step {self.paint_step}: Forcing camera alignment to Validation View #{scribble_val_index}")
            

    #         # 从验证集(val_loader)中获取对应视角的相机参数
    #         # 1. 获取 ViewsDataset 的类实例（通过作者留下的 _data 属性）
    #         val_dataset_instance = self.dataloaders['val']._data
            
    #         # 2. 手动调用 collate 函数生成数据
    #         # collate 期望接收一个索引列表 (batch)，所以传入 [scribble_val_index]
    #         val_data = val_dataset_instance.collate([scribble_val_index])
            
            
    #         # 3. 直接使用 float 数值 (render.py 内部会处理转换)
    #         theta = val_data['theta']
    #         phi_raw = val_data['phi']
    #         radius = val_data['radius']
            
    #         # 手动执行 phi 的偏移计算 (与原代码逻辑保持一致)
    #         phi = phi_raw - np.deg2rad(self.cfg.render.front_offset)
    #         phi = float(phi + 2 * np.pi if phi < 0 else phi)
    #     else:
    #         # === 原有逻辑: 使用训练集视角 ===
    #         theta, phi, radius = data['theta'], data['phi'], data['radius']
    ...
    #     # Render from viewpoint (此时使用的是强制对齐后的视角)
    #     outputs = self.mesh_model.render(theta=theta, phi=phi, radius=radius, background=background)
    #     render_cache = outputs['render_cache']
    #     rgb_render_raw = outputs['image']  # Render where missing values have special color
    #     depth_render = outputs['depth']
    #     # Render again with the median value to use as rgb
    #     outputs = self.mesh_model.render(background=background,
    #                                      render_cache=render_cache, use_median=self.paint_step > 1)
    #     rgb_render = outputs['image']
    #     # Render meta texture map
    ...
    #     # Text embeddings (保持不变)
    #     if self.cfg.guide.append_direction:
    #         # 注意：如果是强制对齐的视角，这里的 data['dir'] 可能不匹配，但通常这里只影响 text prompt，影响不大
    #         # 如果需要严格精确，可以从 val_data['dir'] 获取
    #         if self.paint_step == target_scribble_step:
    #             # 简单处理：使用默认 prompt 或 val_data 的方向
    #             text_z = self.text_z[0] # 假设 batch 为 1
    #             text_string = "Target View" # log用
    #         else:
    #             dirs = data['dir']
    #             text_z = self.text_z[dirs]
    #             text_string = self.text_string[dirs]
    ...
    #     # 防止因 update_ratio 过小而跳过（涂鸦步骤必须执行）
    #     if self.paint_step != target_scribble_step and self.cfg.guide.reference_texture is not None and update_ratio < 0.01:
    #         logger.info(f'Update ratio {update_ratio:.5f} is small for an editing step, skipping')
    #         return
    ...
    #     # ================== 修改 2: 注入涂鸦图像 (Scribble Injection) ==================
    #     if self.paint_step == target_scribble_step:
    #         logger.info(f"Injecting scribble image from {scribble_image_path}...")
    #         try:
    #             # 1. 读取图像
    #             scribble_pil = Image.open(scribble_image_path).convert('RGB')
                
    #             # 2. 强制调整尺寸以匹配 Render Grid Size (例如 1200)
    #             train_grid_size = self.cfg.render.train_grid_size
    #             if scribble_pil.size != (train_grid_size, train_grid_size):
    #                 logger.warning(f"Resizing scribble image from {scribble_pil.size} to ({train_grid_size}, {train_grid_size})")
    #                 scribble_pil = scribble_pil.resize((train_grid_size, train_grid_size), Image.BILINEAR)
                
    #             # 3. 转为 Tensor [1, 3, H, W]
    #             scribble_tensor = torch.from_numpy(np.array(scribble_pil)).float().to(self.device) / 255.0
    #             scribble_tensor = scribble_tensor.permute(2, 0, 1).unsqueeze(0)
                
    #             # 4. 替换 rgb_output
    #             rgb_output = scribble_tensor
                
    #             # 5. 强制全量更新掩码 (Force Update Mask)
    #             # 使用整个物体的掩码，确保涂鸦（包括背景覆盖部分）能完全投影回去
    #             logger.info("Forcing update_mask to match object_mask for proper projection.")
    #             update_mask = outputs['mask'].clone()
                
    #             self.log_train_image(rgb_output, name='injected_scribble_full')
                
    #         except Exception as e:
    #             logger.error(f"Error loading scribble image: {e}")
    #             return
    #     else:
    #         # === 原有 Diffusion 生成逻辑 ===
    #         self.diffusion.use_inpaint = self.cfg.guide.use_inpainting and self.paint_step > 1
